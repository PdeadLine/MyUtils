...
public abstract class BaseServlet extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse resp)
		...
		if(methodName==null || methodName.trim().isEmpty()){
			throw new RuntimeException("您老没有传递method()参数！无法确定您想要调用的方法！");
		}
		 ...
		 *	#我们要查询的是当前类的方法，所以我们需要得到当前类的的class
		 */
			Class c = this.getClass();//获得当前类的Class类对象
			Method method=null;
			try {
			...
			/**
			  *	查看返回的字符串，是否存在冒号，如果没有表示转发（默认）
			  *	如果有，使用冒号分割字符串，得到前缀和后缀！
			  *	其中f，表示转发，如果是r表示重定向，后缀就是转发或重定向的路径了！
			  *
			  **/
			if(result.contains(":")){
				//使用冒号分割字符串，得到前缀和后缀
				int index = result.indexOf(":");//获取”:“的位置
				String s =result.substring(0,index);//截取前缀,表示操作
				String path = result.substring(index+1);//截取后缀，表示路径
				if(s.equalsIgnoreCase("r")){//如果是"r",那么重定向！
